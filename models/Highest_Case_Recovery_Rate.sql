...
select location,year,quarter,month,week,Case_Recovered_Rate,sum(Total_Cases) as Total_Cases ,sum(Total_Deaths) as Total_Deaths,sum(Total_Recovered) as Total_Recovered,sum(Total_Active_Cases) as Total_Active_Cases,
sum(New_Cases) as New_Cases,sum(New_Deaths) as New_Deaths,sum(New_Recovered) as New_Recovered,sum(New_Active_Cases) as New_Active_Cases from source_data where location != 'Indonesia' group by location,year,quarter,month,week,Case_Recovered_Rate order by Case_Recovered_Rate desc,location,year,quarter,month,week
limit 1
